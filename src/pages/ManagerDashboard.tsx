...
export default function ManagerDashboard() {
  ...
  // 1. คำขอที่รอฉันอนุมัติ (เฉพาะลูกทีมในแผนกเดียวกัน)
  const toApprove = getRequestsToApprove(leaveRequests, user).filter(
    (r) => r.department === user.department
  );

  // 2. คำขอของตัวเองที่อยู่ระหว่างรออนุมัติ (ส่งต่อให้ HR/ผู้บริหารพิจารณา)
  const ownPending = getOwnRequests(leaveRequests, user).filter(
    (r) => r.status === 'pending'
  );

  // 3. คำขอลาที่รออนุมัติทั้งหมดเฉพาะในแผนกนี้
  const deptPending = leaveRequests.filter(
    (r) => r.department === user.department && r.status === 'pending'
  );

  // 4. พนักงานในแผนกที่ลาวันนี้ (Approved แล้ว)
  const deptTodayApproved = leaveRequests.filter(
    (r) =>
  ...
  // 5. จำนวนพนักงานทั้งหมดเฉพาะในแผนกนี้
  const deptEmployees = employees.filter((e) => e.department === user.department);
  const totalDeptStaff = deptEmployees.length;

  // 6. ประวัติการอนุมัติ (Audit Log) เฉพาะคำขอของคนในแผนกนี้
  const deptAuditLogs = auditLogs.filter((log) => {
    const targetEmp = employees.find((e) => e.name === log.targetEmployeeName);
    return targetEmp?.department === user.department || log.performedBy === user.id;
  });
  ...
  const handleDecision = (
    ...
    window.setTimeout(() => {
      ...
      if (ok) {
        showToast(
          status === 'approved' ? 'อนุมัติเรียบร้อยแล้ว ✅' : 'ปฏิเสธคำขอเรียบร้อยแล้ว',
  ...
  return (
    <div className="space-y-6">
      {/* ส่วนหัวแดชบอร์ด */}
      <PageHeader
        title={`แดชบอร์ดหัวหน้าแผนก (${user.department})`}
        subtitle={`${user.name} (${user.position}) · ดูแลเฉพาะแผนก ${user.department} · ${formatThaiDate(today)}`}
        action={
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <StatCard
          label="พนักงานในแผนก"
          value={totalDeptStaff}
          tone="default"
          icon="👥"
          hint={`เกณฑ์ขั้นต่ำ ${dept?.minRequiredStaff ?? 0} คน`}
        />
        <StatCard
          label="ลาวันนี้ (approved)"
          value={deptTodayApproved.length}
          tone="warning"
          icon="🌴"
          hint={`ในแผนก ${user.department}`}
        />
        <StatCard
          label="รออนุมัติในแผนก"
          value={deptPending.length}
          tone={deptPending.length > 0 ? 'warning' : 'default'}
          icon="⏳"
        />
        <StatCard
          label="รอฉันอนุมัติ"
      ...
      <section>
        ...
        {snapshot ? (
            ...
            >
              ...
              <p
                className={`mt-1 text-2xl font-bold ${
                  snapshot.isShort ? 'text-rose-700' : 'text-emerald-700'
                }`}
              >
                {snapshot.remaining} / {snapshot.totalEmployees} คน
              </p>
              <p className="mt-1 text-xs text-slate-500">
                ลา {snapshot.onLeave} คน · เกณฑ์ขั้นต่ำ {snapshot.minRequiredStaff} คน
              </p>
              {snapshot.isShort ? (
          ...
          <Card>
            <p className="py-4 text-center text-sm text-slate-500">ไม่พบข้อมูลแผนก</p>
          </Card>
        )}
      </section>

      {/* ---------- คำขอที่ต้องอนุมัติ (ลูกทีมขอลา) ---------- */}
      <section>
        <h2 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-500">
          คำขอที่ต้องอนุมัติ (ลูกทีมในแผนก {user.department})
        </h2>

        {toApprove.length === 0 ? (
          <Card>
            <p className="py-4 text-center text-sm text-slate-500">
              ไม่มีคำขอของลูกทีมที่รออนุมัติในขณะนี้ 🎉
            </p>
          </Card>
        ) : (
      ...
      {/* ---------- คำขอของตนเอง (รอ HR/ผู้บริหารอนุมัติ) ---------- */}
      <section>
        <h2 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-500">
      ...
      <section>
        <h2 className="mb-3 text-sm font-bold uppercase tracking-wide text-slate-500">
          ประวัติการอนุมัติในแผนก (Audit Log)
        </h2>

        <Card className="p-0">
          {deptAuditLogs.length === 0 ? (
            <p className="p-6 text-center text-sm text-slate-500">
              ยังไม่มีประวัติการอนุมัติในแผนกนี้
            </p>
          ) : (
